...
## Architecture
- **Frontend**: Single `py/index.html` containing all HTML, CSS, and JS
- **Backend**: Python WebSocket server (`py/chart_server.py`) relays between browser and serial MCU
- **Charting**: uPlot with two y-axes (Ch A left, Ch B right), common time x-axis
- **Data path**: MCU → serial → Python → WebSocket → browser (base64 ADC → µV)

## Key Files
- `py/index.html` — the entire UI (HTML structure, CSS styling, all JS logic)
- `py/chart_server.py` — Python server that handles serial→WebSocket bridge
...
- **Vertical side-by-side sliders** — gain/bias/trg sliders are vertical (`writing-mode: sideways-lr`), arranged in left/right columns flanking the chart
- **Full-screen fit** — page fills the viewport with zero scrolling (`overflow:hidden` on body and canvas-wrap)
...
```
- Body and `#canvas-wrap` have `overflow:hidden` to prevent page scroll
